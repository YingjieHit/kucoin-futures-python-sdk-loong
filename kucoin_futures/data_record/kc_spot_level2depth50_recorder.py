...
class KcSpotLevel2Depth50Recorder(BaseDataRecorder):
    ...
    def _flush_file_name(self):
        self._file_name = f"kc-spot-level2Depth50-{self._symbol}-{self._cur_date_str}.csv"

    # 订阅数据
    async def _subscribe_data(self):
        self._ws_client = await KucoinWsClient.create(None, self._client, self._deal_msg, private=False)
        await self._ws_client.subscribe(f"/spotMarket/level2Depth50:{self._symbol}")
...
async def main():
    ...
    recorder = KcSpotLevel2Depth50Recorder(
        symbol=symbol,
        file_dir=file_dir,
        max_buffer_size=10
    )
